...
number_of_generating_samples = 1000000  # 生成するサンプル数
...
# 保存
x_generated = pd.DataFrame(x_generated, columns=setting_of_generation.columns, index= range(1000, len(x_generated[:, 0])+1000))
x_generated.to_csv('generated_wine_candidates.csv')  # 生成したサンプルをcsv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
